...
export function buildDiscoveredTelevisionDevice(gladys, television, config = {}) {
  ...
  if (television.model) {
    device.model = television.model;
  }

  device.params.push({
    name: 'location',
    value: television.location,
  });

  if (television.serial) {
    device.params.push({
      name: 'serial',
      value: television.serial,
    });
  }
...
export async function setTelevisionValue({ gladys, client, config, feature, value }) {
  ...
  switch (key) {
    ...
    case FEATURE_KEYS.VOLUME:
      return client.request(WEBOS_COMMANDS.SET_VOLUME, {
        volume: Math.round(Number(value)),
      });

    case FEATURE_KEYS.MUTE:
      return client.request(WEBOS_COMMANDS.SET_MUTE, {
        mute: Boolean(Number(value)),
      });
    ...
    case FEATURE_KEYS.TOAST: {
      ...
      return client.request(WEBOS_COMMANDS.CREATE_TOAST, {
    ...
    case FEATURE_KEYS.SOURCE: {
      ...
      return client.request(WEBOS_COMMANDS.SWITCH_INPUT, {
        inputId,
      });
    }
    ...
    case FEATURE_KEYS.LAUNCH_APP: {
      ...
      return client.request(WEBOS_COMMANDS.LAUNCH_APP, {
        id: appId,
      });
    }
